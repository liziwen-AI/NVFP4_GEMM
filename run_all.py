...
def benchmark(data_list, iters=100):
    # nvidia-smi -lgc 1500
    print(f"🎯 benchmark ,shape:{m,n,k}")    
    ...
    with nvtx_range("gemm_profile_range"):
        out = custom_kernel(_clone_data(data_list[13]))
        torch.cuda.synchronize()
    ...
    # 统计
    times_ms = start_event.elapsed_time(end_event) / iters   
    print(f"平均计时是：{times_ms}")

    return times_ms


if __name__ == '__main__':
    NUM_ITERATIONS_PER_BENCHMARK=100
    ...
    result = benchmark(data_list,NUM_ITERATIONS_PER_BENCHMARK)
